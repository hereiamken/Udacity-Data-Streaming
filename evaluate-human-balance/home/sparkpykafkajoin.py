...
joinStreamingDF = customerRiskStreamingDF.join(emailAndBirthYearStreamingDF, expr("customer = email"))
...
query = joinStreamingDF.selectExpr("TO_JSON(struct(*)) AS value").writeStream \
        .outputMode('append') \
        .format("kafka") \
        .option("kafka.bootstrap.servers", "localhost:9092") \
...
query_console = joinStreamingDF.selectExpr("TO_JSON(struct(*)) AS value").writeStream \
        .outputMode('append') \
        .format('console') \
